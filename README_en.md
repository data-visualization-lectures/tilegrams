...
Netlify is configured via `netlify.toml` to publish the `dist/` directory and to run the build with Node.js `16.20.2`.  
For a “build locally, upload static files” workflow:

1. Build the app locally: `npm run build`
2. Deploy the generated assets: `netlify deploy --prod --dir=dist`

You can also rely on Netlify’s CI to run the same build command automatically on each push.
