...
タイル上の地名ラベルは、サイドバーの「**地名ラベルの表示**」で切り替えられます。

- **自動間引き（重なりを回避）**: 初期設定。ラベル同士が重なる場合、タイル数の多い地域を優先して表示し、重なるものは隠します
- **すべて表示**: すべての地名を表示します（密集地帯では重なることがあります）
- **非表示**: 地名を表示しません

いずれのモードでも、地域にマウスを乗せると正式名称が地図左上に表示されます。
